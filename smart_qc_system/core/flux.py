...
with tab1:
    ...
    with info_col1:
        ...
        part_number = st.text_input("P/N (품명)", value="F2PDN-010-2RPE10")

        option_list = ["F1, R-TYPE C2", "F1, R-TYPE C3", "F2, R-TYPE SR", "F2, R-TYPE FEP", "R-TYPE ", "U-TYPE ", "-"]
        product_option = st.selectbox("옵션", option_list, index=6)
    ...
    dimension_defaults = {
        "길이 치수": {"spec": "270.0", "tol": "1.0"},
        "총 길이 - S to S 치수": {"spec": "20.0", "tol": "1.0"},
        "외경 치수 (OD)": {"spec": "129.0", "tol": "1.0"},
        "내경 치수 (ID)": {"spec": "51.0", "tol": "1.0"},
        "실링 외경": {"spec": "41.8", "tol": "0.3"},
    }
    sample_count = 5
...
with tab2:
    ...
    if db.empty:
        ...
        excel_filename = f"{today_str}_품질이력대장_flux.xlsx"
        pdf_filename = f"{today_str}_품질이력대장_flux.pdf"
        ...
        # 7. 파일 저장 및 다운로드 처리 (탭2 하단)
        save_path = r"C:\smart_qc_system\data\reports\input"
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        # 오늘 날짜로 시작하는 기존 파일들의 개수를 확인하여 순번 결정
        # 엑셀과 PDF가 쌍으로 생성되므로, 2로 나누어 현재 순번을 계산합니다.
        existing_files = [f for f in os.listdir(save_path) if f.startswith(today_str)]
        next_seq = (len(existing_files) // 2) + 1

        # 순번이 포함된 파일명 생성 (예: 260714_품질이력대장_standard_01.xlsx)
        excel_filename = f"{today_str}_품질이력대장_standard_{next_seq:02d}.xlsx"
        pdf_filename = f"{today_str}_품질이력대장_standard_{next_seq:02d}.pdf"

        # 엑셀 및 PDF 버퍼를 지정된 경로에 직접 저장
        ...
        with col1:
            ...
            st.caption(f"저장 경로: {excel_filepath}") # 사용자 확인용 경로 표시
        ...
        with col2:
            ...
            st.caption(f"저장 경로: {pdf_filepath}") # 사용자 확인용 경로 표시